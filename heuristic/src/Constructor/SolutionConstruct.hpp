...
public:
    ...
    // Multi-path Ruin and Recreate Optimization using Simulated Annealing
    ...
    SolutionState parallel_simulated_annealing_ruin_recreate(
        SolutionState& initial_solution, 
        int num_paths = 5,
        int num_threads = 0,  // 0 means use the number of available system threads
        int max_iterations = 500,
        double initial_temperature = 100.0,
        double cooling_rate = 0.98,
        double min_temperature = 0.01,
...
private:
    ...
    ///
    double calculate_avg_flight_hours(const SolutionState& solution);
    int calculate_NFDP_count(const SolutionState& solution);

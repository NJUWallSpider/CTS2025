...
void CrewSchedule::Update_DutyPeriod(){
    ...
    if(duty_period.is_FDuty){
        ...
        //     std::vector<std::pair<std::string, bool>> last_flight_assignments = flight_assignments.at(last_flight.id);
        //     for(auto assignment:last_flight_assignments){
        //         if(assignment.first == crew_.id){
        //             if(assignment.second){
        //                 duty_period.can_be_extended = true;
        //             }
        //             else{
        //                 duty_period.can_be_extended = false;
        //             }
    ...
    // set the previous duties' all unextendable and remove duties with flight time < 5 hours
    auto layover_spots = data_.getLayoverStations();
    for(int i = duty_periods_.size() - 2; i >= 0; i--){
        duty_periods_[i].can_be_extended = 0;
    }

    // // for(int i = duty_periods_.size() - 2; i >= 0; i--){
    // if(duty_periods_.size() <= 1){ return ;}

    // // Get arrival airport of last task in ended duty period
    // std::string arrival_airport;
    // if(duty_periods_[duty_periods_.size() - 2].tasks.empty()){return ;}
    
    // DutyPeriod& second_last_duty_period = duty_periods_[duty_periods_.size() - 2];
    // auto& last_task = second_last_duty_period.tasks.back();
    // arrival_airport = get_arrival_airport(last_task);
    // bool invalid_end = false;
    // Cycle& last_cycle = cycles_.back();

    // while(std::find(layover_spots.begin(), layover_spots.end(), arrival_airport) == layover_spots.end()){
    //     invalid_end = true;
    //     ///////////
    //     if(std::holds_alternative<Flight>(last_task)){
    //         flight_assignments.erase(std::get<Flight>(last_task).id);
    //     }
    //     last_task_end_time_ = get_start_time(last_task);
    //     second_last_duty_period.tasks.pop_back();
    //     //last_cycle.duty_periods.pop_back();
    //     //////////////
    //     /////////////

    //     if(duty_periods_.size() > 2 && second_last_duty_period.tasks.empty()){
    //         duty_periods_.erase(duty_periods_.begin() + duty_periods_.size() - 2);
    //         break;
    //     }
    //     else if(duty_periods_.size() == 2 && second_last_duty_period.tasks.empty()){
    //         break;
    //     }
    //     else{
    //         last_task =  second_last_duty_period.tasks.back();
    //         arrival_airport = get_arrival_airport(last_task);
    //         //////////////
    //         current_airport_ = arrival_airport;
    //     }

    // }

    // if(invalid_end){
    //     duty_periods_.pop_back(); /// the last one 
    //     Update_DutyPeriod();
    // }
